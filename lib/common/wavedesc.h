...
class wavedesc
{
 ...
 public:
  wavedesc(std::string filename);
  void rewind();
  bool next();
  std::vector<std::array<float, 2>> &get() { return data; };
...
wavedesc::wavedesc(std::string filename)
{
...
wavedesc::next()
{
  ...
  for (int i = WAVEDESC.FIRST_VALID_PNT; i < WAVEDESC.LAST_VALID_PNT; ++i) {
    float x = (WAVEDESC.HORIZ_INTERVAL * i + WAVEDESC.HORIZ_OFFSET) * 1.e9;
    float y = (WAVEDESC.VERTICAL_GAIN * DATA[i] - WAVEDESC.VERTICAL_OFFSET) * 1.e3;
    data.push_back({x, y});
  }
  delete [] DATA;
  return true;
