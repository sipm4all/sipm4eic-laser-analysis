...
//float delay = 81.1; // [ns] // NEW
float delay = 80.1; // [ns] // IRR

float threshold = 30.; // [mV]

//const char *tagname = "--Trace--";
const char *tagname = "--Jitter2023-11--";


void
signal_csv(std::string dirname, int nev = kMaxInt)
{
  ...
  while (iev < nev) {
    ...
    TGraph C2(Form("%s/C2%s%05d.txt", dirname.c_str(), tagname, iev), "%lg,%lg");
    if (C2.GetN() == 0) break;
    for (int i = 0; i < C2.GetN(); ++i) {
      C2.GetX()[i] *= 1.e9;
      C2.GetY()[i] *= 1.e3;
    }
    TGraph C3(Form("%s/C3%s%05d.txt", dirname.c_str(), tagname, iev), "%lg,%lg");
    if (C3.GetN() == 0) break;
    for (int i = 0; i < C3.GetN(); ++i) {
      C3.GetX()[i] *= 1.e9;
      C3.GetY()[i] *= 1.e3;
    }
    ...
    auto T2 = waveutils::get_transitions(C2, -400., -1.);
    if (T2.size() != 1) continue;
    auto t2 = T2[0];
    hT2->Fill(t2);

    auto T3 = waveutils::get_transitions(C3, threshold, 1.);
    for (auto t3 : T3) {
      hT3->Fill(t3);
      if (t3 < -0.5 || t3 > 0.5) continue;
      auto amp = waveutils::get_amplitude(C3, t3 - 1.5, t3 + 3.5);
